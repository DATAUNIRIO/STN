...
dygraph(SerieTemporal) %>%
  ...
  dySeries("Taxa_de_cambio_USA", label = "Taxa de Câmbio Real/Dolar média") %>%
...
dygraph(SerieTemporal, main = "Minha primeira série temporal no R") %>%
  dySeries("Taxa_desemprego", label = "Taxa de Desemprego Aberto") %>%
  dySeries("Taxa_de_cambio_USA", label = "Taxa de Câmbio Real/Dolar média") %>%
...
# Como poderíamos colocar essa cor mais fraca?
#main = "Série temporal no R: Tesouro Nacional") %>%
#"#a7b9e8" e "#ed7676"

# Intervalo Horizontal
dygraph(SerieTemporal, main = "Série temporal no R: Tesouro Nacional") %>%
  dySeries("Taxa_desemprego", label = "Taxa de Desemprego Aberto") %>%
  dySeries("Taxa_de_cambio_USA", label = "Taxa de Câmbio Real/Dolar média") %>%
...
# Podemos colocar também linhas verticais
dygraph(SerieTemporal, main = "Série temporal no R: Tesouro Nacional") %>%
  dySeries("Taxa_desemprego", label = "Taxa de Desemprego Aberto") %>%
  dySeries("Taxa_de_cambio_USA", label = "Taxa de Câmbio Real/Dolar média") %>%
...
# Como poderíamos colocar também o Itamar e a Dilma?
